...
```bash
$ helm install --name my-operator rookout/operator --set operator.token=YOUR_ORGANIZATIONAL_TOKEN
```
...
3.  run ``` helm template . --set operator.token=YOUR_ORGANIZATIONAL_TOKEN --name=rookout > rookout-operator.yaml```
